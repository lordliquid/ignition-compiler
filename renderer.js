...
function newInput(newLabel, elementToAppendTo, type) {
    ...
    input.addEventListener('keydown', e => handleValue(e, newLabel));
    label.innerHTML = newLabel;
    return input;
}
...
function handleValue(element, newLabel, div) {
    ...
    console.log(element, newLabel, div);
    if (div) {
...
function execute(config, element) {
    const path = `
  java -jar ${config.signer}
  -keystore=${config.keystore}
  -keystore-pwd=${config.keystorePassword}
  -alias=${config.alias}
  -alias-pwd=${config.aliasPassword}
  -chain=${config.certificate}
  -module-in=${config.path}
  -module-out=${config.path}
  `;
    ...
    exec(path, (error, stdout, stderr) => {
        ...
        success.innerHTML = 'Successfully Signed';
    });
}
